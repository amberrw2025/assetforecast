...
class EconomicDataProvider:
    ...
    def __init__(self):
        ...
        if FRED_AVAILABLE and self.fred_api_key:
            try:
                ...
                print("⚠️ FRED API key invalid, using fallback data")
        else:
    ...
    def _get_fed_funds_rate(self) -> float:
        ...
        try:
            if self.fred:
                ...
                # Fallback: Current approximate rate
                return 5.25
        except:
            return 5.25
    ...
    def get_technical_indicators(self, price_data: pd.Series, periods: int = 14) -> Dict[str, float]:
        ...
        try:
            ...
            # Moving Averages
            indicators['sma_20'] = float(price_data.rolling(window=20).mean().iloc[-1])
            indicators['sma_50'] = float(price_data.rolling(window=50).mean().iloc[-1])
            ...
            indicators = {
                ...
                'bb_position': 0.5,
                'volatility': 0.2,
                'sma_20': float(price_data.iloc[-1]) if len(price_data) > 0 else 100.0,
                'sma_50': float(price_data.iloc[-1]) if len(price_data) > 0 else 100.0,
                'price_to_sma20': 1.0,
                'price_to_sma50': 1.0
            }
    ...
    def get_market_regime(self, indicators: Dict[str, float]) -> str:
        ...
        Args:
            indicators: Economic and technical indicators
    ...
    def get_forecast_adjustments(self, 
        ...
        Args:
            base_forecast: Original model forecast
            economic_indicators: Economic data
            technical_indicators: Technical analysis data
            
        Returns:
            Tuple of (adjusted_forecast, adjustment_details)
        """
