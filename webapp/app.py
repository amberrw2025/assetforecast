...
def create_app():
    ...
    def forecast_plots():
        ...
        plot_files = []
        if plots_dir.exists():
            plot_files.extend([f for f in os.listdir(plots_dir) if f.endswith('.png')])
        
        improved_plot_files = []
        if improved_plots_dir.exists():
            improved_plot_files.extend([f for f in os.listdir(improved_plots_dir) if f.endswith('.png')])
        
        return render_template('forecast_plots.html', 
                             plot_files=sorted(plot_files),
                             improved_plot_files=sorted(improved_plot_files))
